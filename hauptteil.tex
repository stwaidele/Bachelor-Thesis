...
\subsection{Welche Communities gibt es schon?} % (fold)
...
\newpage
\section{Rechtesystem} % (fold)
\label{sec:rechtesystem}

\subsection{Authorisierung} % (fold)
...
% subsection authorisierung (end)

\subsection{Rollen} % (fold)
\label{sub:rollen}
...
% subsection rollen (end)

\subsection{Rechte} % (fold)
\label{sub:rechte}
...
Ein Vorschlag für die Benutzerrechte für die jeweiligen Rollen findet sich im Anhang~\myref{sub:vorschlag_rollen_und_rechte}.
% subsection rechte (end)

% section rechtesystem (end)

\newpage
\section{Technische Aspekte} % (fold)
...
\subsection{Anleitungen und Hilfesystem} % (fold)
...
% subsection anleitungen_und_hilfesystem (end)
...
% subsection uberblick (end)

\subsection{Messgrößen für das Controlling in CSCL–Systemen} % (fold)
...
% subsection messgrossen_fur_das_controlling_in_cscl_systemen (end)
